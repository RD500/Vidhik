...
import { compareTwoDocuments } from '@/app/actions';
import type { Document, HistoryItem } from '@/app/page';
...
export default function DocumentComparison({ session, onComparisonComplete }: DocumentComparisonProps) {
  ...
  const handleCompare = async () => {
    ...
    setIsLoading(true);
    setResults(null);
    const { comparison, error } = await compareTwoDocuments(fileA.content, fileB.content);
    if (error) {
